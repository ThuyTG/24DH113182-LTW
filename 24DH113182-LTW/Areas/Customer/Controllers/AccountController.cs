    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Register(RegisterVM model)
        {
            if (ModelState.IsValid)
            {
                ...
                var user = new User { Username = model.Username, Password = model.Password, UserRole = "Customer" };
                var customer = new _24DH113182_LTW.Models.Customer { CustomerName = model.CustomerName, CustomerEmail = model.CustomerEmail, CustomerPhone = model.CustomerPhone, CustomerAddress = model.CustomerAddress };
                // Link them explicitly so EF knows the FK relationship
                user.Customers.Add(customer);
                // Or: customer.User = user;
                db.Users.Add(user);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult UpdateContact(_24DH113182_LTW.Models.Customer model)
        {
            if (ModelState.IsValid)
            {
                string username = User.Identity.Name;
